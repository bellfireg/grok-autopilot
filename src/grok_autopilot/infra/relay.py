"""
Grok Autopilot — Relay Server
================================
Lightweight API server that receives device tokens from a remote
grok-autopilot client and inserts them into the local 9Router
SQLite database.

Usage (on the server where 9Router runs):
    grok-autopilot relay
...
Install:
    pip install grok-autopilot[relay]
"""
...
RELAY_CONFIG_DIR = Path.home() / ".grok-autopilot"
...
def _insert_token_into_db(
    ...
    with sqlite3.connect(db_path, timeout=10) as conn:
        ...
        # Get next priority
        c.execute(
            "SELECT COALESCE(MAX(priority),0)+1 FROM providerConnections WHERE provider='grok'"
        )
        prio = c.fetchone()[0]
...
def start_relay(
    ...
    Requires: pip install grok-autopilot[relay]
    """
    try:
        import uvicorn
        ...
        print()
        print("  Install with:")
        print("  pip install grok-autopilot[relay]")
        print()
        sys.exit(1)
    ...
    # Build FastAPI app
    app = FastAPI(title="grok-autopilot relay", version="1.1.0")
    security = HTTPBearer()
    ...
    print()
    print(f"  {BOLD}╔══════════════════════════════════════════════════╗{NC}")
    print(f"  {BOLD}║       📡 grok-autopilot relay server            ║{NC}")
    print(f"  {BOLD}╚══════════════════════════════════════════════════╝{NC}")
    print()
    ...
    print(f"    grok-autopilot config set ninerouter-relay-url http://<your-server-ip>:{port}")
    print(f"    grok-autopilot config set ninerouter-relay-token {auth_token}")
    print()
