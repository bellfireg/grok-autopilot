...
def _detect_worker_url(work_dir: Path) -> str | None:
    ...
    try:
        ...
        # "│ xxxxxx  │ your-subdomain.workers.dev │"
        for line in result.stdout.splitlines():
...
def save_worker_url(url: str) -> None:
    ...
    if set_user_config_value("worker_url", url):
        ...
        warn("Could not save config (try: grok-autopilot config set worker-url ...)")
...
def deploy_worker() -> None:
    ...
    try:
        ...
        if worker_url:
            ...
            print()
            print(f"  {CYAN}Run grok-autopilot to start:{NC}")
            print("  grok-autopilot --manual-captcha")
            print()
        else:
            print()
            warn("Worker URL not detected. You can set it manually:")
            print("  grok-autopilot config set worker-url https://your-worker.workers.dev")
            print()
